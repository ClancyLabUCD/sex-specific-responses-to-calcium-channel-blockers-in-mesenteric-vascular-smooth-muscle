...
Vessel Male Model
Usage:	icc -qopenmp SMC_fiber.cpp
...
int main () {
  ...
  //double ITA[] ={-40,	0.00566596,	0.687966,	0.049827,	0.103622,	10.6596,	139.322,	42,	0.000100112,	0.114161,	0.00134746,	0.0139101,	0.0374613,	0.0768319,	0.0041608,	0.0336052};
...
theCell->scaling10 = 50;
    ...
    output = fopen("data_MALE_FINAL.txt", "w");
...
double Calculate_CaL(Cell *theCell, double dt)
{
 ...
 //alpha_act =0.93321*exp(V/12.9372); //Structural changes
  ...
  if ( V == 0 ) {
       J_CaV = 0.22* theCell->dL*theCell->dF*z_Ca * (F) * ( theCell->Ca_in-Ca_out );
  } else {
      J_CaV =0.22 * theCell->dL*theCell->dF* z_Ca *z_Ca *( F) * (F/(R*T)) * V * (( theCell->Ca_in * exp( (z_Ca *F*V)/(R*T)) - Ca_out )/( exp( (z_Ca *F*V)/(R*T) ) - 1 )) ;
    }
